...
class TrainerV2:
    ...
    def _build_trainable_graph(
        ...
        def _add_noise(x):
            shape = [batch_size] + x.shape.as_list()[1:]
            x = x + tf.random.normal(shape=shape, mean=0.0, stddev=stddev)
            return x
        ...
        if add_noise:
            with trainable_graph.as_default():
                model_input = tf.compat.v1.placeholder(
                    tf.float32, model.input_placeholder.shape.as_list(), name='new_input'
                )
                model_input = _add_noise(model_input)
            copy_graph(model.graph, trainable_graph, input_map={model.input_placeholder.name: model_input})
        else:
            copy_graph(model.graph, trainable_graph)
            model_input = get_transformed_tensor(model.input_placeholder, trainable_graph)
