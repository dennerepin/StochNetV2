...
class StochNet:
    ...
    def next_state(
        ...
        next_state = self._scale_back_sum(curr_state_values + state_update_vector)
    ...
    def _scale_back_sum(self, data):
        data *= self.scaler.scale_
        data += 2 * self.scaler.mean_
        return data
    ...
    def generate_traces(
        ...
        # zero step
        traces[0] = self.scale_back(curr_state_values) if curr_state_rescaled else curr_state_values

        # first step: make branch of n_traces samples to continue with
        # next_state ~ [n_traces, n_settings, 1, nb_features]
        next_state_values = self.next_state(
                curr_state_values,
                curr_state_rescaled=curr_state_rescaled,
                round_result=False,
                n_samples=n_traces,
            )
        ...
        # [n_steps, n_traces, n_settings, 1, nb_features] -> [n_steps, n_traces, n_settings, nb_features]
