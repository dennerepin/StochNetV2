...
class Dataset:
    def __init__(self, x, y, bs, drop_remainder=False):
        self.x = x
        self.y = y
        self.bs = bs
...
class Trainer:
        ...
        if 'search' in mode:
            ...
            with tf.compat.v1.Session(graph=trainable_graph) as session:
                ...
                loss_summary_main = tf.compat.v1.summary.scalar(
                    'train_loss_main', tf.reduce_mean(train_operations_main.loss))
                ...
                loss_summary_arch = tf.compat.v1.summary.scalar(
                    'train_loss_arch', tf.reduce_mean(train_operations_arch.loss))

                test_mean_loss_ph = tf.compat.v1.placeholder(tf.float32, ())
                test_loss_summary_main = tf.compat.v1.summary.scalar(
                    'test_mean_loss_main', tf.reduce_mean(test_mean_loss_ph)
                )
                test_loss_summary_arch = tf.compat.v1.summary.scalar(
                    'test_mean_loss_arch', tf.reduce_mean(test_mean_loss_ph))
    ...
    def finetune(
        ...
        with tf.compat.v1.Session(graph=trainable_graph) as session:
            ...
            loss_summary_finetune = tf.compat.v1.summary.scalar(
                'train_loss_finetune', tf.reduce_mean(train_operations_finetune.loss))

            test_mean_loss_ph = tf.compat.v1.placeholder(tf.float32, ())
            test_loss_summary_finetune = tf.compat.v1.summary.scalar(
                'test_mean_loss_finetune', tf.reduce_mean(test_mean_loss_ph))
    ...
    def _build_trainable_graph_search(
        ...
        pred_placeholder = model.pred_placeholder
        ...
        with trainable_graph.as_default():
            ...
            train_input_y = pred_placeholder
    ...
    def _get_datasets(
        ...
        train_ds = Dataset(x_train, y_train, batch_size)
        test_ds = Dataset(x_test, y_test, batch_size)
    ...
    def _train(
        ...
        def _run_test():
            ...
            for X_test, Y_test in test_dataset:
                ...
                fetches = {
                    'loss': train_operations.loss,
                    'pred': 'pred_tensor/Softmax:0'
                }

                res = session.run(fetches=fetches, feed_dict=feed_dict)
                res['loss'] = np.mean(res['loss'])
                test_losses.append(res['loss'])
                predicted_classes = np.argmax(res['pred'], axis=-1)
        ...
        for _ in range(n_epochs):
            ...
            if test_dataset is not None:
                ...
                LOGGER.info(f' - test time: {test_time:.1f} seconds, ACC = {acc}')
