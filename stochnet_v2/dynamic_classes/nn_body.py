...
def cell(
    ...
    with tf.compat.v1.variable_scope(f"{'expand' if expand else 'normal'}_cell_{cell_index}"):
        ...
        for i in range(cell_size):
            tmp = []
            for j in range(2):
                expansion_coeff = expansion_multiplier if expand and indices[2 * i + j] < 2 else 1
                s = state[indices[2 * i + j]]
                with tf.compat.v1.variable_scope(f"mixed_op_{2 if expand else 1}_{2 * i + j}"):
                    out = OP_REGISTRY[op_names[2 * i + j]](s, expansion_coeff)
                tmp.append(out)
